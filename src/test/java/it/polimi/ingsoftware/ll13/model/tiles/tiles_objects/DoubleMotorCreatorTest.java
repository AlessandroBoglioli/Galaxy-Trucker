...
class DoubleMotorCreatorTest {

    private final DoubleMotorCreator creator = new DoubleMotorCreator();

    @Test
    void createTile_shouldReturnDoubleMotorTileWithValidData() throws Exception {
        JSONObject data = createValidDoubleMotorData();

        Tile tile = creator.createTile(data);
        assertInstanceOf(DoubleMotorTile.class, tile);
        DoubleMotorTile doubleMotorTile = (DoubleMotorTile) tile;
        assertAll(
                () -> assertEquals(ConnectorType.SMOOTH, doubleMotorTile.getTopConnector().getType()),
                () -> assertEquals(ConnectorType.DOUBLE, doubleMotorTile.getRightConnector().getType()),
                () -> assertEquals(ConnectorType.SINGLE, doubleMotorTile.getBottomConnector().getType()),
                () -> assertEquals(ConnectorType.UNIVERSAL, doubleMotorTile.getLeftConnector().getType()),
    ...
    void createTile_shouldAlwaysUseBottomDirection(Direction direction) throws Exception {
        ...
        DoubleMotorTile doubleMotorTile = (DoubleMotorTile) creator.createTile(data);
        assertEquals(Direction.BOTTOM, doubleMotorTile.getMotorDirection());
    }
    ...
    // Helper methods
    private static JSONObject createValidDoubleMotorData() {
        JSONObject data = new JSONObject();
        JSONArray connectors = new JSONArray()
